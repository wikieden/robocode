...
- `robocode-core` depends on model, permissions, session, tools, types, and workflows to orchestrate turns and commands.
...
- V2-C memory and task workflows are active on this branch: `robocode-workflows`, `/tasks`, `/task ...`, `/memory ...`, workflow JSONL logs, and resume context.
- V2-B LSP foundation and V2-D richer TUI/structured views are still next.
...
Partial: task workflow depth, terminal UI richness, provider streaming/cancellation maturity, long-session summarization.

Missing: MCP, LSP, skills/plugins, multi-agent/team coordinator, bridge/remote/server mode, automation/cron, voice, managed settings, analytics, feature flags.
