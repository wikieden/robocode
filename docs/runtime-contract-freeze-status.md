...
| Lane facts emitted by core | `runtime_view_state_emits_lane_facts_from_core_store_legacy_lane_statuses` and `legacy_lane_migration_runs_once_at_resume_and_runtime_replays_typed_state` prove one-time TSV migration followed by `lanes.jsonl` replay into `LaneUpdated` runtime facts without TUI reads | Done |
