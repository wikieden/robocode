...
**目标：** 在不破坏现有 session、permission 与 transcript 不变量的前提下，为 Viden 增加项目级 tasks、双层 memory，以及 workflow 导向的 resume context。

**架构：** 本阶段新增 `viden-workflows` crate，内部拆成 `tasks`、`memory`、`resume_context`、`stores` 四个模块。`viden-session` 继续作为 transcript 的事实源；workflow 状态保存在 append-only workflow event logs 和派生 SQLite index 中；`viden-core` 负责把 workflow 命令接入现有 command、permission 与 transcript 主链路。

**技术栈：** Rust workspace crates、JSONL append-only logs、SQLite derived indexes、现有 Viden REPL / command runtime
...
- Viden 能创建、更新、阻塞、关联、归档、恢复项目级 task。
- Viden 能直接写 session memory，并通过 suggest/confirm 流程维护 project memory。
...
- 新增：`viden-workflows/Cargo.toml`
- 新增：`viden-workflows/src/lib.rs`
- 修改：`Cargo.toml`
- 修改：`viden-types/src/lib.rs`

- [ ] 在 `Cargo.toml` 中把 `viden-workflows` 加入 workspace members。
- [ ] 创建 `viden-workflows/Cargo.toml`，依赖 `viden-types`、`serde` 以及仓库里已使用的最小持久化辅助库。
- [ ] 在 `viden-workflows/src/lib.rs` 中先导出空模块：
  - `tasks`
  - `memory`
  - `resume_context`
  - `stores`
- [ ] 在 `viden-types/src/lib.rs` 中增加 workflow 共享类型：
  - `TaskId`
  - `MemoryId`
  - `TaskStatus`
  - `TaskPriority`
  ...
  - `MemorySource`
  - `MemoryStatus`
- [ ] 在 `viden-types/src/lib.rs` 中增加数据结构：
  - `TaskRecord`
  - `MemoryEntry`
...
- [ ] 保持这些新类型与现有 session/transcript 模式兼容，至少包含：
  ...
  - 在适合时增加 `PartialEq`
  - 在适合时增加 `Eq`
- [ ] 在 `viden-types/src/lib.rs` 中为新 enum 增加 CLI / serde roundtrip 单测。
- [ ] 运行：`cargo test -p viden-types`
...
- 新增：`viden-workflows/src/stores.rs`
- 修改：`viden-session/src/lib.rs`
- 修改：`viden-types/src/lib.rs`

- [ ] 在 `viden-workflows/src/stores.rs` 中增加 storage-path helpers，从当前 cwd 与 session home 推导 per-project workflow home。
- [ ] 复用 `viden-session` 已有的 project-key 约定，不引入第二套项目 identity。
...
- [ ] 如果缺少必要的 project-key 或 path helper，可在 `viden-session/src/lib.rs` 中暴露，但不要把 workflow 状态挪进该 crate。
- [ ] 在 `viden-workflows/src/stores.rs` 中补测试：
  - path derivation
  - JSONL append/load roundtrip
  - SQLite rebuild from event logs
- [ ] 运行：`cargo test -p viden-workflows stores`
...
- 新增：`viden-workflows/src/tasks.rs`
- 修改：`viden-workflows/src/lib.rs`
- 修改：`viden-types/src/lib.rs`

- [ ] 在 `viden-workflows/src/tasks.rs` 中定义 task-domain commands/events：
  - create
  - update
  - status change
  - link dependency
...
- [ ] 运行：`cargo test -p viden-workflows tasks`
...
- 新增：`viden-workflows/src/memory.rs`
- 修改：`viden-workflows/src/lib.rs`
- 修改：`viden-types/src/lib.rs`

- [ ] 在 `viden-workflows/src/memory.rs` 中定义 memory-domain commands/events：
  - add
  - suggest
  - confirm
  - reject
...
- [ ] 运行：`cargo test -p viden-workflows memory`
- [ ] 在 memory 行为与测试稳定后提交。
...
- 新增：`viden-workflows/src/resume_context.rs`
- 修改：`viden-workflows/src/lib.rs`
- 修改：`viden-session/src/lib.rs`
- 修改：`viden-types/src/lib.rs`

- [ ] 在 `viden-workflows/src/resume_context.rs` 中增加 resume-context builder，输入包括：
  - current task state
  - memory state
...
- [ ] 运行：`cargo test -p viden-workflows resume_context`
- [ ] 在 resume-context 行为确定后提交。

## Task 6：把 workflow runtime 接入 Viden core

**文件：**

- 修改：`viden-core/src/lib.rs`
- 修改：`viden-permissions/src/lib.rs`
- 修改：`viden-types/src/lib.rs`
- 修改：`Cargo.toml`

- [ ] 在需要的位置增加 `viden-workflows` 依赖。
...
- [ ] 在 `viden-permissions/src/lib.rs` 中增加 workflow 写操作的权限集成：
  - reads default-allow
...
- Viden 拥有新的 `viden-workflows` crate，内部包含 task、memory、resume-context、store 模块。
