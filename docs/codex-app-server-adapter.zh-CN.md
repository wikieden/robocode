...
Viden 0.1.7 把 Codex 当作第一个 host-delegate agent backend。目前实现已经可以
启动 Codex CLI jobs、跟踪 log/result、在 TUI 里显示 active work、提取 resume/file
evidence，并把 write-capable delegation 放到 Viden permission gate 后面。
...
生成的 schema 已经包含 Viden 真正 adapter 所需的关键部分：
...
Viden 应把 app-server 数据映射到现有 host-delegate lifecycle：
...
| approval requests | Viden permission prompt 和 transcript permission log |
...
7. 已完成：`/agent probe codex --turn-write <task>` 只作为带环境变量保护的
   ...
   workspace-write turn 可能在 Viden 收到 approval request 前直接修改文件。
...
9. 已完成：approval-like server requests 会写入 JSONL evidence，并返回
   decline/no-grant responses，避免 app-server work 卡住或绕过 Viden
...
- write-capable launch 前必须经过 Viden permission approval；
- `.viden/agents/` job records、logs、results、baseline status 和 evidence
  extraction。
...
的后续工作。fixture 证明的是 Viden ingestion/display path，不代表真实模型每个
live turn 都会稳定发出这些事件。2026-05-27 的 disposable live write probe 证明
Codex Desktop 可以通过 `mcpToolCall` 在没有先发出 Viden approval request 的情况下修改
workspace；Viden 现在会把它记录为 `mcp-tool-call`、`mcp-tool-completed` 和
