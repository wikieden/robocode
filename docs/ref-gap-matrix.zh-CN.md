...
| LSP | 语言服务器集成与推荐逻辑 | 已在 main 落地，但深度仍是 partial：已有 real semantic queries、session reuse、document sync、normalized output | 与本地工作流整合的语义级代码智能 | 中 | V2 | 已进入 main；仍比参考平台更轻 |
| Skills | 可复用 workflow system | 未开始 | 本地 skill discovery 与执行模型 | 高 | V3 | 行为相似，Rust 原生实现 |
| Plugins | 内置和第三方插件加载 | 通用 plugin 系统未开始；provider plugins 已纳入 provider-system 目标 | 带清晰信任边界的 plugin loading 和管理 | 高 | V2 / V3 | 要区分 provider plugins 与更广义的 skills/plugin 平台 |
...
| Memory | persistent memory 支持 | 已在 main 落地，但深度仍是 partial：已有 project/session memory、suggestion confirmation、event logs | 与长期工作流绑定的显式 memory 模型 | 中 | V2 | 已进入 main；仍比参考平台更轻 |
