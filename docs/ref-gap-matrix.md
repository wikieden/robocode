...
| Memory | Persistent memory support | V2-C active / partial implementation with project and session memory, suggestion confirmation, and event logs | Explicit memory model tied to long-lived workflows | Medium | V2 | Implemented on `codex/v2-memory-task-workflows`; still lighter than the reference platform |
| Tasks | Task creation and workflow management | V2-C active / partial implementation with lifecycle reducer, blockers, archive/restore, and resume context | Task lifecycle integrated into sessions and later agents | Medium | V2 | Implemented on `codex/v2-memory-task-workflows`; agent integration remains future work |
