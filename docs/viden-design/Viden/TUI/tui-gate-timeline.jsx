...
function CommandGate({cmd, consequence, risk, total=50, onResolve}){
  ...
  return (
    <div className="ovl" onClick={()=>finish({decision:'deny',reason:'click-out'})}>
      <div className="gatebox" onClick={e=>e.stopPropagation()}>
        <div className="gh">⚠ <span style={{whiteSpace:'nowrap'}}>{tGT('Command gate','命令闸')}</span><span className="rk">{risk||tGT('ELEVATED','高危')}</span></div>
        <div className="gsub">{tGT('The agent wants to run a shell command that can\u2019t be undone. Authorize how?','agent 要执行一条不可撤销的 shell 命令。如何授权?')}</div>
        <div className="gcmd"><span className="pr">$ </span>{cmd}</div>
        <div className="gcsq">↳ {consequence}</div>
        <div className="gopts">
          {GATE_OPTS.map((o,i)=>(
            <div key={o.key} className={"gopt "+(sel===i?'on ':'')+(o.deny?'deny':'')}
                 onMouseEnter={()=>setSel(i)} onClick={()=>finish({decision:o.key})}>
              <span className="n">{o.n}</span><span className="lb">{o.label}</span><span className="sub">{o.sub}</span>
            </div>
          ))}
        </div>
        <div className="gfoot">
          <span><kbd>↑↓</kbd> {tGT('select','选')}</span>
          <span><kbd>1</kbd>–<kbd>4</kbd> {tGT('direct','直选')}</span>
          <span><kbd>⏎</kbd> {tGT('confirm','确认')}</span>
          <span className="sp"></span>
          <span className="cd">{tGT('auto-deny','超时拒')}&nbsp;<span style={{minWidth:'26px',display:'inline-block',textAlign:'right'}}>{left}s</span><span className="bar"><i style={{width:pct+'%'}}></i></span></span>
        </div>
      </div>
    </div>
  );
