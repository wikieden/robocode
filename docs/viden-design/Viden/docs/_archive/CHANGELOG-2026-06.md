# Viden Changelog 归档 · 2026-06（窗口外整段原样保真 · 主文件见 docs/CHANGELOG.md）

## 2026-06-28
- [设计规范] **新增 `GUI/gui-kit.css` canonical 套件(对标 tui-kit.css)**:把 D1 同源 GUI chrome(窗口壳/标题栏/活动 rail/lane 行/Environment/状态栏/输入区/边缘浮出/细滚动条)收编为单一类名词汇,token-only。DESIGN-REF GUI 段加套件总纲;存量页 D1–D13 逐步迁移(同 TUI 并存策略),新页直接用。
- [设计规范] **DESIGN-REF 登记 cockpit 交互件**:活动 rail(`.act`/`.actbtn` + 底部 pin 切换)、lane 行(`.wslane.s-*` + `AgentLogo` codex/claude/kiro/viden)、Environment 复合面板(`.envsec`/`.envhd`/`.envrow`/`.envctx`/`.todorow`)、边栏自动隐藏+边缘浮出(`.edgewrap`/`.floatpanel`,float 3 轨 / pinned 4 轨 / focus 2 轨规则)——从"草稿"转正,各带最小 HTML。
- [设计规范] Core 展示页组件补最小 HTML 登记:`.rcard`/`.gcard`/`.callout`/`.toktable`/`.ladder`+`.lrow`。
- [GUI] **D1 清除「屏幕适配」死代码**:`AspectModes` 函数(JSX ~217 行)+ `.k4*`/`.uw*`/`.pt*`/`.ptd*`/`.asphead` CSS(~207 行)整段移除(此前已下线渲染),-42.8KB;DiagnosticsView / 状态栏 / ActivityRail 等保留,载入无报错。

## 2026-06-26
- [GUI] **D1 lanes 浮出动效:从活动 rail 右缘弹出**:浮层入场距离由 `translateX(-115%)`(≈-271px,从屏幕最左横扫、盖过整排图标)改为 `translateX(-14px)`(短距滑入 + 淡入),静止位置不变(锚定 rail 右侧);隐藏态加 `pointer-events:none` 防误触。左右 edgewrap 同步。
- [GUI] **D1 活动 rail 加 pin/float 切换键**:在常驻可见的 52px 活动 rail 底部(Settings 上方)加 `IPin` 切换——float(自动隐藏)态点击 → pinned 常驻并高亮、再点 → 回 float;始终可见、易发现,补齐"浮动 ↔ 钉住"显式切换(原仅浮出面板内 `.pinbtn`,收起时不可见)。
- [GUI] **D1 lanes 边栏默认自动隐藏(float)+ 修 float 列布局 bug**:`leftMode` 默认 `pinned`→`float`,lanes 栏默认收起,鼠标移到活动 rail 右侧细边条(`.edgewrap.l`)自动浮出、移开 700ms 后收回(原 1500ms);pin 键可改回常驻。顺带修复 float 模式潜伏 bug——`cols` 在未 pin 时仍留 `0px` lanes 轨道,导致 centerwrap 落进 0 宽轨、Environment 误占 1fr;改为按实际子元素数生成轨道(pinned 4 轨 / float 3 轨 / focus 2 轨)。
- [GUI] **竖屏 4K 驾驶舱:lanes 边栏可隐藏 + 边缘悬停浮出**:focus 键收起 lanes 栏(`data-side=hidden`,grid 中列归 0),移到活动 rail 右侧 `.edgepeek` 细条即滑出浮层、移开收回;与 D1 Focus 同源交互。
- [GUI] **竖屏 4K 驾驶舱重做 · 元素全面对齐 D1 横屏**:窗口 chrome 改用 D1 同源真实元素——标题栏(dots+wordmark+projsel pill+gitops chips+tbtools 图标组)、52px 活动 rail(`.act`/`.actbtn` + IChat/IWorktree/IReview/IEvid/IDiag)、`.wslane` lane 行(真实 AgentLogo:codex/claude/kiro/viden + s-need/s-work/s-done 状态边框)、全窗宽 `.statusbar`(gear + ┆ ticker + 右 warning)、`.composer`/`.cbox`、Environment 浮层改用 `.envsec`/`.envhd`/`.envrow`/`.envctx`/`.todorow` 词汇(Environment/Context/Todo/Sources)。竖向布局:活动 rail + lanes 常驻 · 对话↑ → 监控+终端 dock↓。
- [GUI] **新增「竖屏 4K 驾驶舱」预览页**(`Viden - 竖屏 4K 驾驶舱 (GUI).html`):参考 Codex 竖屏布局,固定 1080×1920(4K 竖屏半尺)letterbox scale-to-fit;纯静态 HTML(canonical,可直接编辑),窗口壳 `--win-edge`/`--r-xl`/`--shadow-win` + 主题化细滚动条沿用规范。
- [GUI] **D1 删除「屏幕适配」演示段**:移除 `<AspectModes/>`(4K/超宽/竖屏三帧)渲染调用,该 section 从 D1 页面下线(函数/CSS 暂留为死代码,无渲染)。
- [GUI] **D1「屏幕适配」三帧预览重做(去拥挤 / 提可读)**:`.k4win`(4K 全景)/`.uwwin`(21:9 超宽)原先被强制 16:9/21:9 窄高比塞 5 列 → 全面截断、过密;改为**固定高度**(4K 580px、超宽 452px)+ 放宽列宽(chat 列加权,subagents 收窄)+ 字号/留白上调(行 9.5→10.5px、状态栏 20→26px),精简过长表头(chat 去 provider 名,subagents 去 fan-out)。窗口壳沿用 `--win-edge`/`--r-xl`/`--shadow-win`;竖屏帧 `.ptdesk` 保持并随之协调。
- [GUI] **D1 屏幕适配预览窗口壳对齐主 cockpit**:`.k4win`/`.uwwin`/`.ptdesk` 圆角(11/12/14px)+ 投影(裸 rgba)统一到 `var(--r-xl)` + `var(--shadow-win)`。
- [GUI] **弱视觉主题化细滚动条全 GUI 铺开(D1 同源)**:`*` 全局 `scrollbar-width:thin` + `scrollbar-color`,WebKit 9px 透明轨道 / `--border-soft` 拇指(2px 内缩更纤细)/ 悬停增强到 `--border`;替换系统默认白色滚动条,13 个 GUI 页全覆盖。
  - [GUI] 顺带修 D1 `.wsscroll` 隐式横向白条(补 `overflow-x:hidden`)。
- [GUI] **窗口级底部状态栏抽成 `<GuiStatusBar>`(gui-statusbar.jsx)并铺满桌面屏**:此前仅 D1 有底栏,其余页窗口底缘各不相同;现抽出 28px 等宽 canonical 底栏(左 ⚙ + 滚动 ticker + 右状态段,母版=D1 `.statusbar`),作为 `.frame`/`.win` 最后子节点贴底。已铺 D2决策/D2存档/D4/D5/D7/D9/D11/D12/D13;D1 保留其交互版不替换;D2横/D3 召唤坞、D10(双独立窗口 showcase)、Pip 有意不铺。
  - [设计规范] DESIGN-REF GUI 段登记 `<GuiStatusBar>`(接入方式 + props + 已铺/未铺清单);class 前缀 `gui-sb`,样式自注入,token-only。
  - [GUI] 审计确认顶部标题栏度量(46/gap13/pad14/bg-void/border-soft/灯12)早已全页一致,仅类名 `.titlebar`/`.vbar`/`.winbar` 三种等价实现,本次未改。
- [GUI] **主窗口骨架列宽对齐 D1 + token 化**:新增 `--rail-act`(52)/`--rail-left`(218)/`--rail-right`(300) 作单一真源,7 个 GUI 页的活动 rail / 左 lanes 栏 / 右 context rail 一律引用,消除散落 px(右栏原 308/318/332/372→300,D2决策左栏 282→218,D2存档队列栏 264→218)。
  - [设计规范] 例外登记:D9 右栏 420(4 列 GPU 监视器,有意加宽)、D4/D11 向导步骤栏 236(非 lanes 栏)保持原样。
  - [文档] DESIGN-REF Token 速查表加「骨架列宽」行,`.frame` 条目补主窗口骨架规范 + 例外说明。
- [GUI] **窗口外壳统一对齐 D1 浮空规范**:13 个 GUI 页的窗口壳(`.win`/`.frame`)边框/半径/投影收敛到 D1 同一套——边 `--border`→`--win-edge`、半径 12/13/14px→`--r-xl`、平投影 `shadow-lg`/`.55`→D1 重浮空投影。原 D1/D2横屏/D3 已是浮空,本次把 D2决策/D4/D5/D7/D9/D11/D12/D13/D2存档(平投影)与 D10(12px/.55)拉齐。
  - [设计规范] tokens.css 新增 `--shadow-win`(D1 浮空窗口投影 `0 40px 120px -40px … ,0 0 0 1px …`)作唯一真源,消除各页裸 `rgba()` 投影。
  - [文档] DESIGN-REF 窗口壳条目与阴影 token 表同步(`.win`/`.frame`/`.winbar` 共用 `--win-edge` + `--r-xl` + `--shadow-win`)。
...
- [TUI] 11 个旧展示页终端框全部迁到 tui-kit canonical `.v*`:消除 `.term/.emu/.term-chrome/.tbar` + `.tlt/.lt/.tnm/.emunm/.nm/.ti/.dot` 类名分叉,并接入 `tui-kit.css`。
  - 同步删除内联红绿灯 hex(改 `.vlights><i.a/.b/.c>`);保留 depth256/ANSI 演示、`.dep` 徽标、`.emu` 仅作降透明度修饰符。
  - 9 页迁移(T4/Charm 无终端框);逐页核验渲染(opencode 4 屏 / T5 保真 / T0 驾驶舱)+ 0 残留 fork 引用;DESIGN-REF 旧类名标注废弃。
