...
- cost-blind 通道（terminal、tmux）的 lane 绝不显示推断出的 token 或金额。其 inspector
  ...
  绝不渲染成功。墙钟时间按操作者可读的量级渲染：不足一秒用毫秒，不足一分钟用带一位小数
  的秒，超过一分钟用分加秒。

## 监督决策中心

决策中心（`Ctrl-G`、`/decisions`）按固定顺序列出 Core 正在等待的决策：待处理的工具审批、
merge gate、仍待裁决的 review request、仍待重新验证的 conflict bounce。已裁决的 review 与
已解决的 bounce 属于历史，不作为决策列出。行使用登记过的 TUI 字形 `⏸`（gate）、`◌`（等待）
与 `⚠`（冲突）；记录类型标记（`GATE`、`REVIEW`、`CONFLICT`）在各语言下保持一致，因此
Core 标识符始终可用同一个标签找到。

选择审批行仍然打开固定的 Approval 覆盖层，行为不变。选择 gate、review 或 conflict 行则打开
监督决策覆盖层：

- 动作由每帧重新读取的完整 Core 记录推导，绝不使用精简行。开放的 gate 提供接受与拒绝；
  存在待处理冲突的 gate 在源 Lane 清除冲突前，用重新验证取代接受；已合并的 gate 只提供
  回滚，并附带明确的不可逆提示；待裁决的 review 提供两种结论。对该记录当前状态永远不适用
  的动作根本不会列出。
- 仅在本地看起来不满足 Core 前置条件的动作仍然会发送。是否允许该决策只由 Core 裁定。
- 拒绝、回滚与回弹必须填写原因；两种 review 结论的反馈都是选填，与 Core 命令契约一致。
  必填原因为空，或文本超过 Core 的 500 字符 trust-text 上限时，在本地拒绝并给出提示且不
  发送任何命令——客户端绝不会截断后再发送半句原因。
- actor 与 reviewed-evidence 绑定按 GUI 集成门适配器相同的推导方式，从 Core 自身记录中回放。
  当无法回放出所有者或规范证据时，该动作在本地被拒绝，因为没有可发送的合法载荷。
- 方向键或数字键选择动作，`Enter` 确认。`Esc` 按顺序退栈：先关闭原因输入行，再关闭覆盖层。
  关闭覆盖层绝不取消在途的 Core 命令，pending 徽标会继续显示。该覆盖层没有文本过滤器，
  因此在流式回合中，非动作数字的可打印字符仍然编辑输入框。
- 结果回显同时出现在覆盖层与状态栏：`◌` 表示 pending 并附带 command id，`✓` 表示已确认，
  `✗` 表示被拒绝并原样展示 Core 给出的原因。已裁决的结果在发起下一次监督动作或关闭覆盖层
  时重置为 idle；pending 状态绝不自动重置。
- 回执丢失或从未发布会占死唯一的在途槽位，因此覆盖层与决策中心页脚都提供放弃动作。放弃
  只停止本地归属跟踪：它不裁决任何结果，也不会取消 Core 命令。

## 审计时间线

审计时间线覆盖层是监督闭环的只读一侧：决策中心回答“Core 还在等什么”，它回答
“这里已经发生过什么”。它消费 Core 审计契约
（`RuntimeCommand::QueryAudit` -> `RuntimeEventKind::AuditPageLoaded`），本版本
是纯只读的——逐行浏览不携带任何动作。

有两个入口打开它，它没有自己的全局快捷键：

- 监督决策覆盖层中的 `Audit trail` 行，范围限定为当前屏幕上的记录（gate 或
  conflict 目标审计 `merge_gate:<gate id>`，review 目标审计
  `review_request:<review id>`，均使用契约自身的对象 kind 常量）；
- 决策中心页脚的 `Audit timeline (project)` 选项，以无范围方式打开项目时间线。
  Core 的审计存储已经限定在本项目自己的 workflow 目录内，因此查询不设置
  `project_id` 或 `lane_id` 过滤，而不是自行捏造一个。

行为：

- 每次打开都会发起一次全新的首页查询（`limit` 100），并在覆盖层关闭时丢弃面板，
  因此重新打开的时间线绝不会是一页年龄不明的旧数据。`QueryAudit` 不产生任何变更，
  也不触发权限提示，因此该时间线在 Plan 模式下同样可读。
- 记录按由新到旧渲染为 `{time} {action} {outcome} {objects} {args}`。`time` 为
  `HH:MM:SS` UTC——审计记录是跨机器比对的证据，随地区偏移的时钟会让两个读者对同一个
  事实产生分歧。`action` 是 Core 原始的点分 key（`gate.decided`、
  `handoff.created`），刻意不做本地化；契约特意让它不含自然语言，读者才能对两条
  时间线做 diff。结果使用注册字形 `✓`（成功）与 `✗`（拒绝或失败）；本构建不认识的
  结果渲染为字面 ASCII `?`，而不是借用一个它从未拥有的状态。objects 渲染为逗号连接
  的 `kind:id` 对，args 渲染为空格连接的 `k=v` 对，两者均按显示宽度截断。
- 四种状态保持可见地互相区分：loading（查询在途且尚无数据到达）、empty（Core 返回
  了空页）、error（`✗` 并原样展示 Core 给出的原因）以及已加载列表。页脚始终说明已
  加载多少条记录以及是否还有更早的记录，因此短列表绝不会被误读为整条时间线。
- 方向键在各行之间移动选择，并可移动到 `Load older records` 行，`Enter` 确认该行。
  时间线完整时该行被隐藏。已有一页在途时的第二次请求会在本地被拒绝并给出提示，不发送
  任何命令。在记录行上确认是有意的空操作。
- 该读取与监督决策槽位独立关联：在途的 gate、review 或 revert 命令既不会阻塞时间线，
  也不会被某一页裁决。覆盖层没有文本过滤器，因此可打印字符在流式回合中仍然编辑输入框。
- `Esc` 关闭并回到基础状态。TUI 只为 Global Jump 保留覆盖层返回路径，因此该时间线的
  退出方式与从决策中心打开的审批覆盖层完全一致。

已知限制，如实记录而非掩盖：`AuditPageLoaded` 不携带 command id，因此另一个客户端并发
查询产生的页可能被归属到本面板在途的查询上。对单操作者闭环而言这是可以接受的——该页
仍然是真实的 Core 页，覆盖层关闭时即被丢弃，下一次查询会自行纠正。彻底消除这一歧义属于
Core 契约变更（在页上携带 command id），而不是客户端的猜测。
