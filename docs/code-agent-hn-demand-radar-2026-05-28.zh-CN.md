...
这份文档用 Hacker News 讨论刷新 RoboCode 的竞品认知。它不是热度榜，而是需求雷达：
开发者在抱怨什么、认可什么，以及 RoboCode 还缺什么。
...
- RoboCode 应该增加轻量 spec/steering loop，再做更强自治。
...
- RoboCode 有 tasks、memory 和 release plans，但还没有产品内 spec-driven workflow。
...
- RoboCode 有 per-lane worktree 方向和 review/apply safety，但还没有结构化 test-data
...
- Adapter doctor 应该说明哪些配置属于 RoboCode，哪些属于 agent-native。
- 不要无理由复制 secret 或重复 MCP config。

缺口：

- RoboCode 目前只规划 ACP probes，还没有具体兼容目标，例如“跑一个 ACP server 并把
...
- RoboCode 有 context pressure 和 provider health，但没有 cost ledger、quota forecast
...
- RoboCode setup 已避免保存 secret，但还没有 MCP、external API 或 agent adapters 的
...
- RoboCode 有 extension boundary 规划，但还没有 hook lifecycle 或 hook evidence model。

## 竞品差距矩阵

| 竞品 / pattern | 强信号 | RoboCode 缺口 | 产品回应 |
| --- | --- | --- | --- |
| Claude Code | 成熟 terminal loop、MCP、hooks、skills、subagents、checkpoints、non-interactive mode | RoboCode 有更强 cockpit 目标，但内建 automation surface 更弱 | 增加 hook lifecycle、spec/steering 和可复现 Claude lane |
| Codex | 本地 Rust CLI、强安装体验、跨 surface 方向、evidence 预期 | RoboCode 暂时不应替代 Codex | 把 Codex 做成 reference delegated lane backend |
| Zed | ACP external agents、editor-native threads、worktree parallelism | RoboCode 缺 ACP runtime 和 editor-native file context | P0 lane 稳定后做 ACP probe/event mapping；TUI 继续做 ops cockpit |
| Kiro | Specs、steering files、hooks、MCP、privacy-first framing | RoboCode 缺产品内 spec/steering workflow | 增加 task envelope spec phases 和 project steering files |
| Kilo / OpenClaw | 多 surface、很多模型、cloud/slack/automations | RoboCode 目前 TUI-first、local-first | 不急追 cloud；先做 cost/rate ledger 和 automation boundary |
| Aider | Git-native 简洁体验和 repo map | RoboCode context 仍可能过于 transcript-centric | 把 compact repo/project map 接入 ContextBundle |
| OpenHands / Goose | Platform/SDK 形态，CLI + GUI + API | RoboCode runtime 仍主要 TUI-led | 保持 core 可复用，但 API/server 等 TUI 稳定后再做 |
| DeepSeek-TUI | dense terminal-native provider experience | RoboCode 编排目标更强，但 terminal UX 仍需持续打磨 | 继续把确定性截图和 live provider smoke 作为 release gate |
...
RoboCode 的 TUI-first 策略仍然成立，但前提是副屏真正成为 evidence 和 control surface，
而不是 dashboard。
