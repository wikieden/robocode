...
- 前端代码只从 `viden-core` 导入 transport-neutral `CoreClient` 边界和公共
  ...
  或 workflow 内部模块。
- 本地前端通过 `viden_core::LocalCoreHost` 打开项目。它会 canonicalize
  已存在的 workspace 目录，运行共享 runtime bootstrap，启动 `RuntimeSupervisor`，
  并返回已绑定的 `CoreClient`。重新绑定到另一 workspace 会创建独立 binding 和
  stream，不能改变已有 client 的 cursor 或 snapshot。
