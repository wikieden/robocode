pub(crate) const ANTHROPIC_API_BASE: &str = "https://api.anthropic.com";
pub(crate) const DEEPSEEK_API_BASE: &str = "https://api.deepseek.com";
pub(crate) const DEEPSEEK_ANTHROPIC_API_BASE: &str = "https://api.deepseek.com/anthropic";
pub(crate) const GROQ_API_BASE: &str = "https://api.groq.com/openai/v1";
pub(crate) const KIMI_API_BASE: &str = "https://api.moonshot.ai/v1";
pub(crate) const MISTRAL_API_BASE: &str = "https://api.mistral.ai/v1";
pub(crate) const OPENAI_API_BASE: &str = "https://api.openai.com";
pub(crate) const OPENROUTER_API_BASE: &str = "https://openrouter.ai/api/v1";
pub(crate) const OLLAMA_API_BASE: &str = "http://localhost:11434";
pub(crate) const QWEN_API_BASE: &str = "https://dashscope.aliyuncs.com/compatible-mode/v1";
pub(crate) const TOGETHER_API_BASE: &str = "https://api.together.xyz/v1";
pub(crate) const VOLCENGINE_API_BASE: &str = "https://ark.cn-beijing.volces.com/api/v3";
pub(crate) const ZHIPU_API_BASE: &str = "https://open.bigmodel.cn/api/paas/v4";
