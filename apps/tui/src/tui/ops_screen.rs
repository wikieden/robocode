...
mod tests {
    ...
    fn ops_evidence_rows_reuse_agent_task_runtime_view() {
        ...
        state.entries.push(TuiEntry {
            ...
            body: "shell command=cargo test -p viden-cli".to_string(),
        });
        state.lanes.truncate(1);
    ...
    fn ops_tests_lsp_rows_surface_test_and_diagnostics_evidence() {
        ...
        state.entries.push(TuiEntry {
            ...
            body: [
                ...
                "  status: failed",
                "  exit code: 101",
                "  command: cargo test -p viden-cli ops_",
                "  duration: 42ms",
                "  failure summary:",
                "    - assertion failed",
            ]
        ...
        assert!(rendered.contains("TEST    failed  42ms  cargo test -p viden-cli ops_"));
    ...
    fn ops_evidence_rows_prioritize_test_command_and_failure() {
        ...
        assert!(rendered.contains("evidence command cargo test -p viden-cli ops_"));
