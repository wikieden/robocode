...
smoke_dir=$(mktemp -d /tmp/viden-codex-app-server-smoke.XXXXXX)
cleanup() {
...
output=$(
  printf '%s\n%s\n%s\n' \
    '/agent probe codex --turn Say exactly VIDEN_APP_SERVER_SMOKE_OK and do not edit files.' \
    '/agent status' \
    '/quit' |
